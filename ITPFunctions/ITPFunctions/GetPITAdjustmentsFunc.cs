...
{
    public static class GetPITAdjustmentsFunc
    {
        [FunctionName("GetPITAdjustmentsFunc")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "GetPITAdjustment")] HttpRequest req,
            ILogger log)
        {
            ...
            using (SqlConnection conn = new SqlConnection(str))
            {
                ...
                if (name.Equals("PIT") && !string.IsNullOrEmpty(EntityId))
                {
                    text = "SELECT * FROM [WorkPaper].[vw_PITAdjustments_Workpaper]" +
...

        {
            ...
            DataTable dataTable = new DataTable { TableName = "PITAdjustments" };
